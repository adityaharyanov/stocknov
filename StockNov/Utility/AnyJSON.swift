...
class AnyJSON: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        if var arrayContainer = try? decoder.unkeyedContainer() {
            self.value = try AnyJSON.decodeArray(from: &arrayContainer)
        } else if var container = try? decoder.container(keyedBy: MyCodingKey.self) {
            self.value = try AnyJSON.decodeDictionary(from: &container)
        } else {
            let container = try decoder.singleValueContainer()
            self.value = try AnyJSON.decode(from: container)
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        if let arr = self.value as? [Any] {
            ...
            try AnyJSON.encode(to: &container, array: arr)
        } else if let dict = self.value as? [String: Any] {
            var container = encoder.container(keyedBy: MyCodingKey.self)
            try AnyJSON.encode(to: &container, dictionary: dict)
        } else {
